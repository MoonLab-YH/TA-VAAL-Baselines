...
# Custom
from config import *
from models.query_models import VAE, Discriminator2, GCN
        ...
        for iter_count in range(train_iterations):
            ...
            if iter_count == 0:
                ...
                r_u_0 = torch.from_numpy(np.random.uniform(0, 1, size=(unlabeled_imgs.shape[0], 1))).type(
                    torch.FloatTensor).cuda()
                with torch.no_grad():
                    _, embedding_l, features_l = task_model(labeled_imgs)
                    _, embedding_u, feature_u = task_model(unlabeled_imgs)
            else:
                with torch.no_grad():
                    _, embedding_l, features_l = task_model(labeled_imgs)
                    _, embedding_u, feature_u = task_model(unlabeled_imgs)
            ...
            for count in range(num_vae_steps):  # num_vae_steps
                ...
                labeled_preds = discriminator(embedding_l)
                unlabeled_preds = discriminator(embedding_u)

                lab_real_preds = torch.ones(labeled_imgs.size(0))  # 1
                unlab_real_preds = torch.zeros(unlabeled_imgs.size(0))  # 0

                lab_real_preds = lab_real_preds.cuda()  # 1
                unlab_real_preds = unlab_real_preds.cuda()  # 0
            ...
            for count in range(num_adv_steps):
                ...
                lab_real_preds = lab_real_preds.cuda() # 1
                unlab_fake_preds = unlab_fake_preds.cuda()  # 0
...
def query_samples6(model, method, data_unlabeled, subset, labeled_set, cycle, args):
    if method == 'Random':
    ...
    if method == 'TA-VAAL':
        ...
        all_set = torch.tensor([True]*50000)
        for i in range(len(all_set)):
            if i in labeled_set:
                all_set[i] = False
        unlabeled_set = all_set.nonzero()[:,0].tolist()
        # unlabeled_loader = DataLoader(data_unlabeled, batch_size=BATCH, sampler=SubsetSequentialSampler(unlabeled_set), pin_memory=True)
        unlabeled_loader = DataLoader(data_unlabeled, batch_size=BATCH, sampler=SubsetSequentialSampler(subset), pin_memory=True)
        if args.dataset == 'fashionmnist':
            vae = VAE(28, 1, 3).cuda()
            discriminator = Discriminator2(28).cuda()
        else:
            vae = VAE().cuda()
            discriminator = Discriminator2(512).cuda()
    ...
    print(f'mean of arg is {arg.float().mean()}')
    return arg
